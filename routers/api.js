    		...
    		let etherString = ethers.utils.formatEther(balance);
				if (parseFloat(etherString) > 0.0001){
					provider.getGasPrice().then(function(gasPrice) {
						tokenContract.functions.AddSale(sale_id, user_email, sale_coin, sale_amount, sale_masternode_id, {
							gasPrice: gasPrice,
							gasLimit: 65000,
						}).then(function(txid) {
							res.jsonp({
  ...
  router.post('/recordrewords', (req, res) => {
      ...
      console.log(sale_id, user_email, sale_coin, sale_amount, sale_masternode_id);
    		...
    		let etherString = ethers.utils.formatEther(balance);
				if (parseFloat(etherString) > 0.0001){
					provider.getGasPrice().then(function(gasPrice) {
						tokenContract.functions.AddReward(sale_id, user_email, sale_coin, sale_amount, sale_masternode_id, {
							gasPrice: gasPrice,
							gasLimit: 65000,
						}).then(function(txid) {
							res.jsonp({
